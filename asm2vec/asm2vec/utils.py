...
def load_data(paths, limit=None):
    """
        从路径中加载functions和tokens
        Return:
            functions: 路径中包含的函数构成的function列表
            tokens: 全部函数中包含的token的toekens
    """
...
def preprocess(functions, tokens):
    """
        预处理，x为目标seq前后各一句ins组成的Tensor（前面第一位代表函数编号），y为目标token对应的Tensor
        Return:
            x: 目标seq前后各一句ins各部分的token索引组成的Tensor（前面第一位代表函数编号）[batch_size,7(2*3+1)]
            y: 目标指令各部分token所用组成的向量 ,[batch_size,3]
    """
    x, y = [], []
...
def train(
    ...
    for epoch in range(epochs):
        ...
        model.train()
        for i, (inp, pos) in tqdm(enumerate(loader)):
