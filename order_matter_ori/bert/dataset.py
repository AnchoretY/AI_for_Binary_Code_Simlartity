...
LastEditors: Yhk
LastEditTime: 2022-07-21 10:52:10
Description: 
'''
import os
...
class BERTDataset(Dataset):
    ...
    """
    def __init__(self, cfg_corpus_path, append_data_path,vocab, seq_len, max_samples=-1,encoding="utf-8"):
        """
        ...
        self.append_data_path = append_data_path
        self.append_data = read_pickle(append_data_path)
        self.big_label = 1   #块同图任务生成是否同图样本
    ...
    def __getitem__(self, item):
        ...
        block1_content,block2_content,bert_big_label = self.get_BIG_sample()
        block1_content = [self.vocab.sos_index] + [self.vocab.stoi.get(c, self.vocab.unk_index) for c in block1_content.split()] + [self.vocab.eos_index]
        block2_content = [self.vocab.stoi.get(c, self.vocab.unk_index) for c in block2_content.split()] + [self.vocab.eos_index]
        
        bert_big_input = (block1_content + block2_content)[:self.seq_len]
        cfg_padding = [self.vocab.pad_index for _ in range(self.seq_len - len(bert_big_input))]
        bert_big_input.extend(cfg_padding)

        big_segment_label = ([1 for _ in range(len(block1_content))] + [2 for _ in range(len(block2_content))])[:self.seq_len]
        big_segment_label.extend(cfg_padding)
        
        # 任务4：BIG两个基本块是否在一个图中
        block1_content,block2_content,optimize_type = self.get_GC_sample()
        block1_content = [self.vocab.sos_index] + [self.vocab.stoi.get(c, self.vocab.unk_index) for c in block1_content.split()] + [self.vocab.eos_index]
        block2_content = [self.vocab.stoi.get(c, self.vocab.unk_index) for c in block2_content.split()] + [self.vocab.eos_index]
        
        bert_gc_input = (block1_content + block2_content)[:self.seq_len]
        cfg_padding = [self.vocab.pad_index for _ in range(self.seq_len - len(bert_gc_input))]
        bert_gc_input.extend(cfg_padding)

        gc_segment_label = ([1 for _ in range(len(block1_content))] + [2 for _ in range(len(block2_content))])[:self.seq_len]
        gc_segment_label.extend(cfg_padding)
        ...
        output = {
                ...
                "bert_big_input": bert_big_input,
                "bert_big_segment_label": big_segment_label,
                "bert_big_label":bert_big_label,
                "bert_gc_input":bert_gc_input,
                "bert_gc_segment_label":gc_segment_label,
                "bert_gc_label":optimize_type,
                }

        return {key: torch.tensor(value) for key, value in output.items()}

    def get_BIG_sample(self):
        """
            获取
        """
        dice = random.random() 
        blocks = random.sample(random.choice(self.append_data)[0],2)
        if dice > 0.5:
            blocks = random.sample(random.choice(self.append_data)[0],2)
            return blocks[0],blocks[1],1
        else:
            block1 = random.choice(random.choice(self.append_data)[0])
            block2 = random.choice(random.choice(self.append_data)[0])
            return block1,block2,0

        
    def get_GC_sample(self):
        data = random.choice(self.append_data)
        optimize_type = OPTIMIZE_MAP[data[1]]
        block_contents = random.sample(data[0],2)
        return block_contents[0],block_contents[1],optimize_type
